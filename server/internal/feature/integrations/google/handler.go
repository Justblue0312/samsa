...
func (h *HTTPHandler) Authorize(w http.ResponseWriter, r *http.Request) {
	...
	stateData := map[string]any{
		"provider": string(sqlc.OauthProviderGoogle),
	}
	if err := common.StoreState(r.Context(), h.rdb, state, sqlc.OauthProviderGoogle, stateData, h.cfg.OAuth2.OAuthStateTTL); err != nil {
		respond.Error(w, apierror.Internal())
		return
	}
...
func (h *HTTPHandler) Callback(w http.ResponseWriter, r *http.Request) {
	...
	stateData, err := common.RetrieveState(r.Context(), h.rdb, state, sqlc.OauthProviderGoogle)
	if err != nil {
		respond.Error(w, apierror.BadRequest("invalid or expired state"))
		return
	}

	if stateData["provider"] != string(sqlc.OauthProviderGoogle) {
	...
	userInfo, err := h.useCase.ProcessCallback(r.Context(), token.AccessToken, token.RefreshToken, expiresAt, amd)
	if err != nil {
		respond.Error(w, apierror.Internal())
		return
	}

	_ = common.DeleteState(r.Context(), h.rdb, state, sqlc.OauthProviderGoogle)
